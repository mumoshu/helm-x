...
func NewSecretsStorage(tillerNs string) (*ReleaseStorage, error) {
...
func NewConfigMapsStorage(tillerNs string) (*ReleaseStorage, error) {
	clientset, err := kube.New(nil).KubernetesClientSet()
	if err != nil {
		return nil, fmt.Errorf("Cannot initialize Kubernetes connection: %s", err)
	}
...
func (s *ReleaseStorage) GetLatestRelease(name string) (*rspb.Release, error) {
	return s.driver.Last(name)
}

func (s *ReleaseStorage) AdoptRelease(name, ns, manifest string) error {
...
func (s *ReleaseStorage) GetRelease(name string) (*rspb.Release, error) {
